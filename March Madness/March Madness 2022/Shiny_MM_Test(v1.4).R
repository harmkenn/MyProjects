...
SeedSum <- gt(seed.history,rownames_to_stub =TRUE)%>% 
    data_color(
        columns = 1:7, 
...
ui <- dashboardPage(
    dashboardHeader(title = "Shiny March Madness Test v1.4",titleWidth = "450px",
    ...
    dashboardSidebar(width = 150,
                     sidebarMenu(
                         ...
                         menuItem("Elastic Predict", tabName = "elastic_predict")
                     ) #################### End sidebarMenu
    ...
    dashboardBody(
        ...
        tabItems(
            ...
            ), ################### End Full Predict Tab
            ########################### Start Elastic Predict Tab
            tabItem("elastic_predict",
                    fluidRow(
                        column(width = 12,
                               box(title = "Predicting Eleastic Bracket", width = NULL, status = "primary",
                                   sliderInput("sldr_year_e_p","Year",2008,2021,2021,step=1,width = 500,ticks = FALSE),
                                   textOutput("txt_ESPN_e"),
                                   formattableOutput("tbl_elastic_predict")     
                               ) ############# End box
                        ), ############## End column      
                    ) ################# End of fluidrow
            ) ################### End Elastic Predict Tab
        ) ##################### End tabItems
    ) ####################### End dashboard Body
) ######################### End dashboard Page
...
server <- function(input, output, session) {
    ...
    observeEvent(input$sldr_year,{
        ...
        bracket[3,6] <- games$Winner[nrow(games)]
    ...
    ########################### Start of Full Predict Tab
    observeEvent(input$sldr_year_e_p,{
        pyear <- input$sldr_year_e_p
        if (pyear == 2020) {pyear <- 2021}
        FUGames <- read.csv("FUAllGames.csv")
        stage1 <- FUGames %>% filter(Year >= 2008) %>% select(c(-3))
        stage1$amv <- stage1$A.F.Score - stage1$A.U.Score
