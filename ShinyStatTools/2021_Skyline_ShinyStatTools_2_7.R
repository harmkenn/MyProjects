...
data.quant <- data.frame(matrix(numeric(), nrow=500, ncol=1))
...
data.Chi.gof <- data.frame(matrix(numeric(), nrow=2, ncol=9))
colnames(data.Chi.gof) <- c("X1","X2","X3","X4","X5","X6","X7","X8","X9")
rownames(data.Chi.gof) <- c("Obs","Ratio")
...
ui <- dashboardPage(
  dashboardHeader(title = "Shiny Stat Tools v2.7",titleWidth = "450px",
              ...
              fluidRow(
                column(width = 3,
                       box(title = "Selections", width = NULL, solidHeader = TRUE,
                           ...
                           numericInput("st_df", "Degrees of fredom:",2),
...
server <- function(input, output, session) {
    ...
    if(sum(!is.na(quant.in$values[,1]))>1){
      ...
      if(!is.null(tryCatch(ggplot(quant.df), error = function(e){}))){
        ...
        if(input$ck.dot == TRUE){
          qs.plots <- qs.plots +
            geom_jitter(aes(x = "", y = quant.x), color= "red", size = 2, height = 0, width =.1) +
  ...
  observeEvent(c(input$f_t2p,input$st_sym), {
    ...
    t2pp <- st.df %>% ggplot(aes(x,y))+geom_line()+
      geom_area(aes(y=y),alpha=0) + scale_x_continuous(sec.axis = sec_axis(~.*1, name = "A",breaks = seq(-5*1,5*1,1)),breaks = seq(-5,5)) +
      theme(axis.title.y = element_blank(),axis.text.y = element_blank(),axis.ticks.y = element_blank()) +
