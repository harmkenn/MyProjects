...
ws <- JoinedLoserStats %>% select(starts_with("w_")) 
ls <- JoinedLoserStats %>% select(contains("l_")) %>% select(5:35)
dwl <- ws - ls
dwl <- dwl %>% rename_all(funs(str_replace(., "w.", "d."))) %>% tibble::rowid_to_column("ID")
