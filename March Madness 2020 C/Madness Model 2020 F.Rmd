...
AG0320stacked <- read.csv("AG0320stacked.csv")
teamstats <- read.csv("SRstats0320.csv")
teamcomb <- left_join(AG0320stacked,teamstats,by=c("Year"="Year","Team"="School"))
...
AG0318F <- teamcomb %>% filter(Favored == "F" & Year != pyear & Year != 2020)
AG0318U <- teamcomb %>% filter(Favored == "U" & Year != pyear & Year != 2020)

#Get the difference of the just stats
AG0318FS <- AG0318F %>% select(7,10,16:61)
AG0318US <- AG0318U %>% select(7,10,16:61)
dStats0318 <- AG0318FS - AG0318US

# Set seed and create assignment

assignment <- sample(1:2, size = nrow(dStats0318), prob = c(.7,.3), replace = TRUE)

# Create a train, validation and tests from the original data frame 
NCAA_train <- dStats0318[assignment == 1, ]    # subset NCAA to training indices only
NCAA_test <- dStats0318[assignment == 2, ]   # subset NCAA to test indices only
...
The64 <- teamcomb %>% filter(Year == pyear & Round == 1) %>% select(1,3:8,10,16:61)
...
#Get the difference of the just stats
The64FS <- The64F %>% select(6,9:54)
The64US <- The64U %>% select(6,9:54)
...
# Need to assign F and U to Predictions from round 1
The32 <- teamcomb %>% filter(Year == pyear & Round == 1 & Team %in% pred1) %>% select(1,3:8,10,16:61)
The32$GameN <- rep(33:48, each=2)
The32 <- The32 %>% arrange(GameN, desc(SRS7))
...
#Get the difference of the just stats
The32FS <- The32F %>% select(6,9:54)
The32US <- The32U %>% select(6,9:54)
...
The16 <- teamcomb %>% filter(Year == pyear & Round == 1 & Team %in% pred2) %>% select(1,3:8,10,16:61)
The16$GameN <- rep(49:56, each=2)
The16 <- The16 %>% arrange(GameN, desc(SRS7))
...
#Get the difference of the just stats
The16FS <- The16F %>% select(6,9:54)
The16US <- The16U %>% select(6,9:54)
...
The8 <- teamcomb %>% filter(Year == pyear & Round == 1 & Team %in% pred3) %>% select(1,3:8,10,16:61)
The8$GameN <- rep(57:60, each=2)
The8 <- The8 %>% arrange(GameN, desc(SRS7))
...
#Get the difference of the just stats
The8FS <- The8F %>% select(6,9:54)
The8US <- The8U %>% select(6,9:54)
...
The4 <- teamcomb %>% filter(Year == pyear & Round == 1 & Team %in% pred4) %>% select(1,3:8,10,16:61)
The4 <- The4 %>% arrange(desc(Region), SRS7)
...
#Get the difference of the just stats
The4FS <- The4F %>% select(6,9:54)
The4US <- The4U %>% select(6,9:54)
...
The2 <- teamcomb %>% filter(Year == pyear & Round == 1 & Team %in% pred5) %>% select(1,3:8,10,16:61)
The2$GameN <- rep(63, each=2)
The2 <- The2 %>% arrange(GameN, desc(SRS7))
...
#Get the difference of the just stats
The2FS <- The2F %>% select(6,9:54)
The2US <- The2U %>% select(6,9:54)
...
summary(track)
#cbind(pred1,pred2,pred3,pred4,pred5,pred6)
```
