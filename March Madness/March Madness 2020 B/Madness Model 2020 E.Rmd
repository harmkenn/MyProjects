...
teamstats <- read.csv("NCAASeasonTeamStats00to19.csv")
...
# Dump MP and PF
teamcomb <- teamcomb %>% select(-c(MP,PF))
...
AG0218F <- teamcomb %>% filter(Favored == "F" & Year != pyear)
AG0218U <- teamcomb %>% filter(Favored == "U" & Year != pyear)

#Get the difference of the just stats
AG0218FS <- AG0218F %>% select(7,10,11,18:46)
AG0218US <- AG0218U %>% select(7,10,11,18:46)
dStats0218 <- AG0218FS - AG0218US

# Set seed and create assignment

assignment <- sample(1:2, size = nrow(dStats0218), prob = c(.9,.1), replace = TRUE)

# Create a train, validation and tests from the original data frame 
NCAA_train <- dStats0218[assignment == 1, ]    # subset NCAA to training indices only
NCAA_test <- dStats0218[assignment == 2, ]   # subset NCAA to test indices only
...
The64 <- teamcomb %>% filter(Year == pyear & Round == 1) %>% 
  select(1,5,6,7,8,11,18:46)
The64$GameN <- rep(1:32, each=2)
The64 <- The64 %>% arrange(GameN, desc(AdjEM))
The64$Favored <- rep(c("F","U"),32)
...
#Get the difference of the just stats
The64FS <- The64F %>% select(4,6:35)
The64US <- The64U %>% select(4,6:35)
The64DS <- The64FS - The64US

#Combine the F and U Leading Columns with dif stats
LThe64F <- The64F %>% select(1:5)
LThe64U <- The64U %>% select(2:5)
New64 <- cbind(LThe64F,LThe64U,The64DS)
colnames(New64)[9]<-"U.Team"
...
The32 <- The32 %>% arrange(GameN, desc(AdjEM))
...
#Get the difference of the just stats
The32FS <- The32F %>% select(4,6:35)
The32US <- The32U %>% select(4,6:35)
The32DS <- The32FS - The32US

#Combine the F and U Leading Columns with dif stats
LThe32F <- The32F %>% select(1:5)
LThe32U <- The32U %>% select(2:5)
New32 <- cbind(LThe32F,LThe32U,The32DS)
colnames(New32)[9]<-"U.Team"
...
The16 <- The16 %>% arrange(GameN, desc(AdjEM))
...
#Get the difference of the just stats
The16FS <- The16F %>% select(4,6:35)
The16US <- The16U %>% select(4,6:35)
The16DS <- The16FS - The16US

#Combine the F and U Leading Columns with dif stats
LThe16F <- The16F %>% select(1:5)
LThe16U <- The16U %>% select(2:5)
New16 <- cbind(LThe16F,LThe16U,The16DS)
colnames(New16)[9]<-"U.Team"
...
The8 <- The8 %>% arrange(GameN, desc(AdjEM))
...
#Get the difference of the just stats
The8FS <- The8F %>% select(4,6:35)
The8US <- The8U %>% select(4,6:35)
The8DS <- The8FS - The8US

#Combine the F and U Leading Columns with dif stats
LThe8F <- The8F %>% select(1:5)
LThe8U <- The8U %>% select(2:5)
New8 <- cbind(LThe8F,LThe8U,The8DS)
colnames(New8)[9]<-"U.Team"
...
The4 <- teamcomb %>% filter(Year == pyear & Round == 1 & Team %in% pred4) %>% 
  select(1,5,6,7,8,11,18:46)
The4 <- The4 %>% arrange(desc(Region), AdjEM)
...
#Get the difference of the just stats
The4FS <- The4F %>% select(4,6:35)
The4US <- The4U %>% select(4,6:35)
The4DS <- The4FS - The4US

#Combine the F and U Leading Columns with dif stats
LThe4F <- The4F %>% select(1:5)
LThe4U <- The4U %>% select(2:5)
New4 <- cbind(LThe4F,LThe4U,The4DS)
colnames(New4)[9]<-"U.Team"
...
The2 <- The2 %>% arrange(GameN, desc(AdjEM))
...
#Get the difference of the just stats
The2FS <- The2F %>% select(4,6:35)
The2US <- The2U %>% select(4,6:35)
The2DS <- The2FS - The2US

#Combine the F and U Leading Columns with dif stats
LThe2F <- The2F %>% select(1:5)
LThe2U <- The2U %>% select(2:5)
New2 <- cbind(LThe2F,LThe2U,The2DS)
colnames(New2)[9]<-"U.Team"
