...
etrade_all <- read.csv("etrade_all.csv")
...
```{r trans}
Etrade_Tx <- read.csv("Etrade_Tx.csv")
...
```{r Grph}
Ffig <- MFValue[1:251,] %>% ggplot() + geom_point(aes(x = Date, y = sum)) + 
...
Ffig <- MFValue %>% select(c(1,4:8)) %>%
  pivot_longer(., cols = c(2:6), names_to = "symbol", values_to = "Val") %>%
  ggplot(aes(x = Date, y = Val, color = symbol), size = .001) +
  geom_point()
