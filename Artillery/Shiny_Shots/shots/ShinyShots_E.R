...
observeEvent(input$get_sol, {
  ...
  VI <- T_alt-F_alt
  RangeD <- data.frame(Range=range,Alt=F_alt)
  AOSd <- atan((VI)/range)
  AOS <- AOSd * 3200/pi
  ...
  c.elev <- glm(Elev~poly(Range,6,raw=TRUE) + poly(Alt,6,raw=TRUE), data = MACs_1)
  c.TOF <- glm(TOF~poly(Range,6,raw=TRUE)+ poly(Alt,6,raw=TRUE), data = MACs_1)
  c.drift <- glm(Drift~poly(Range,6,raw=TRUE)+ poly(Alt,6,raw=TRUE), data = MACs_1)
  c.maxord <- glm(Maxord.z~poly(Elev,6,raw=TRUE)+ poly(Alt,6,raw=TRUE), data = MACs_1)
  c.csf.p <- glm(csf.p~poly(Range,6,raw=TRUE)+ poly(Alt,6,raw=TRUE), data = MACs_1)
  c.csf.n <- glm(csf.n~poly(Range,6,raw=TRUE)+ poly(Alt,6,raw=TRUE), data = MACs_1)
  c.range <- glm(Range~poly(Elev,6,raw=TRUE)+ poly(Alt,6,raw=TRUE), data = MACs_1)
  ...
  MaxOrd <- as.numeric(predict(c.maxord, data.frame(Elev=QE, Alt = F_alt), type = "response"))
  MaxOrd.p <- MaxOrd
  
  TOF <- as.numeric(predict(c.TOF, data.frame(Range=range, Alt = F_alt), type = "response"))
  drift <- as.numeric(predict(c.drift, data.frame(Range=range, Alt = F_alt), type = "response"))
  CR <- as.numeric(predict(c.range, data.frame(Elev=QE, Alt = F_alt), type = "response"))
  if (AOS == 0){CR <- range}
  ...
  FM <- data.frame(cbind(c(range,round(CR,0),round(az,1),round(gd,1),round(drift,1),round(defl,1),"M795",
                           chg,MACs_1$MV[1],round(Elev,1),round(AOS,1),round(CAS,1),round(site,1),
                           round(QE,1),round(TOF,1),round(MaxOrd.p,0))))
  colnames(FM) <- c("Fire Mission")
  rownames(FM) <- c("Range (Meters)","Corrected Range (Meters)","Azimuth to Target (mils)","Grid Declination","Drift (mils)","Deflection (mils)","Shell","Charge","Muzzle Velocity (m/s)","Elevation (mils)","Angle of Site (mils)","Complementary Angle of Site (mils)","Site (mils)","QE (mils)","TOF (seconds)","MaxOrd (MSL Meters)")
...
# TAB 5 Artillery Physics
observeEvent(input$get_sol5, {
  v0 <- input$MV # initial velocity in m/s 
  am0 <- input$QE # QE in mils
  m <- (input$sw)/2.20462 #mass lbs to kg
  ...
  traj <- data.frame(matrix(ncol = 7))
  colnames(traj)=c("Time s","k","Vel m/s","Angle r","Angle mils","Range m","Alt m")
  t <- t0
  v <- v0
  th <- th0
  x <- x0
  ...
  while (y > y0 - 1){
    ...
    x <- x + v*dt*cos(th)
    y <- y + v*dt*sin(th)
    nextrow <- c(t,k,v,th,th*3200/pi,x,y)
    traj <- rbind(traj,nextrow)
  }
  FM <- data.frame(cbind(c(round(x,0),round(t,1),round(th*3200/pi,1),round(v,0),round(max(traj$`Alt m`),0))))
  colnames(FM) <- c("Results")
  rownames(FM) <- c("Range (Meters)","Time of Flight (seconds)","Angle of Fall (mils)","Final Velocity (m/s)","MaxOrd (MSL Meters)")
