...
Rankings <- as_tibble(read.csv("NCAARankings.csv"))
```

```{r Cds}
Games2000_19 <- all_Games %>% filter(Year > 2000) %>% tibble::rowid_to_column("ID") %>% as_tibble()
wss <- SeasonStats %>% mutate(Year = as.integer(str_extract(Season, "^.{4}")) + 1) %>% setNames(paste0("h_",names(.)))
