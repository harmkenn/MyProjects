...
Here is the combination of all data

```{r result}
all02 <- left_join(games85,KP02, by = c("Winner"="Team", "Year"="Year"))%>%filter(Year>2001)
all02 <- left_join(all02,PASE, by = c("Winner"="TEAM"))
all02 <- left_join(all02,teamstats00, by = c("Winner"="Team", "Year"="Year")) %>% select(1:12,15:45)
all02 <- left_join(all02,KP02, by = c("Loser"="Team", "Year"="Year"))
all02 <- left_join(all02,PASE, by = c("Loser"="TEAM"))
all02 <- left_join(all02,teamstats00, by = c("Loser"="Team", "Year"="Year")) %>% select(1:47,50:80)
