...
nsq <-  plyr::round_any(Northing,100000,floor)/100000 #Grab off the first two digits off the Northing
  if((gzn %% 2) == 0) {nsq <- nsq +5} #Even grid zones increase the letter by 5
...
LL2UTM2mgrs<-function(lng,lat){
  ...
  lz <- lgzl[lzc]
  esq <- plyr::round_any(Easting,100000,floor)/100000 #Grab the first digit off of easting
  emod <- mod(gzn,3) # Zone mod 3 for 100km lookup
  if (emod == 0) {emod <- 3} #turn mod zero into row 3
  esql <- esqls[emod,esq] #Here is the 100km easting grid letter
  nsq <-  plyr::round_any(Northing,100000,floor)/100000 #Grab off the first two digits off the Northing
  if((gzn %% 2) == 0) {nsq <- nsq +5} #Even grid zones increase the letter by 5
...
longv <- (sample.int(36000,size=5000,replace=TRUE)-1)/100-180
latv <- (sample.int(16000,size=5000,replace=TRUE)-1)/100-80
...
alln <- data.frame(hem = NA, gzn = gzn, gzl = gzl, gsnl = gsnl, ngsn = NA)
fulln<-match_df(northdes,alln,on=c("gzn","gzl","gsnl"))
