---
title: "Madness Model Predict 11-19"
author: "Ken Harmon"
...
AG1119stacked <- read.csv("AG1119stacked.csv")
teamstats <- read.csv("SRstats1119.csv")
teamcomb <- left_join(AG1119stacked,teamstats,by=c("Year"="Season","Team"="School"))
...
AG1119F <- teamcomb %>% filter(Favored == "F" & Year != pyear)
AG1119U <- teamcomb %>% filter(Favored == "U" & Year != pyear)

#Get the difference of the just stats
AG1119FS <- AG1119F %>% select(7,10,15:86)
AG1119US <- AG1119U %>% select(7,10,15:86)
dStats1119 <- AG1119FS - AG1119US

# Set seed and create assignment

assignment <- sample(1:2, size = nrow(dStats1119), prob = c(.9,.1), replace = TRUE)

# Create a train, validation and tests from the original data frame 
NCAA_train <- dStats1119[assignment == 1, ]    # subset NCAA to training indices only
NCAA_test <- dStats1119[assignment == 2, ]   # subset NCAA to test indices only
