...
ui <- dashboardPage(
  dashboardHeader(title = "Shiny Stat Tools v1.1",titleWidth = "450px",
...
server <- function(input, output, session) {
  ...
  observeEvent(eventExpr = input$plot, {
    ...
    if(sum(!is.na(desc.in$values[,1]))>1){
      ...
      plmin <- min(hist.x) - binwidth
      plmax <- max(hist.x) + binwidth
      hist.df <- data.frame(hist.x)
      if(!is.null(tryCatch(ggplot(hist.df), error = function(e){}))){
        dd1 <-  plot_ly(x = hist.x, type = "histogram", nbinsx = bins,
                        marker = list(color = "skyblue2", line = list(color = "darkgray", width = 2)))
      } #Eif
      if(!is.null(tryCatch(ggplot(hist.df), error = function(e){}))){
        dd2 <- plot_ly(x = hist.x, type = "box", showlegend = FALSE) %>% 
          layout(yaxis = list(showticklabels =FALSE), xaxis = list(title="", range = c(plmin,plmax)))
      } #Eif
      output$hist <- renderPlotly({
         subplot(dd1, dd2, nrows = 2, heights = c(0.6, 0.3)) %>% 
          layout(showlegend = FALSE,xaxis = list(title="", range = c(plmin,plmax)))
      }) #Eoutput$hist
